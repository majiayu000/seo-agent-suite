...
2. Run the baseline collector:

```bash
python3 ../../scripts/repo_seo_baseline.py --root . --json
```

3. If a public site exists, run:

```bash
python3 ../../scripts/site_meta_audit.py <homepage> --json
```

4. Search exact brand, repo, package, and docs-site names when search visibility is the question.
5. Record repo, docs site, package registry, and release discoverability separately.
6. Read `../../references/data-sources.md` before recommending paid APIs or MCP providers.
